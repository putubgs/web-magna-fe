...
type EventPopUpProps = {
  open: boolean;
  close: () => void;
  save: (superAdminEventManagementData: SuperAdminEventManagementProps, index: number) => void;
  delete: (index: number) => void;
  data: SuperAdminEventManagementProps[];
  index: number;
};

export default function SuperAdminEventDetailManagementPopUp({
  open,
  close,
  save,
  delete: deleteData,
  ...
  const [editRegistrationUrl1, setEditRegistrationUrl1] = useState<boolean>(true);
  ...
  const [editRegistrationUrl2, setEditRegistrationUrl2] = useState<boolean>(true);
  ...
  const tooltipData = [
	...
	useEffect(() => {
		if (data && data.length > 0) {
			...
			setRegistrationUrl2(data[0].registrationUrl2 ?? "");
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const editEventData: SuperAdminEventManagementProps = {
      organization,
      eventName,
      registrationUrl1,
      date,
											...
											<tbody>
												<tr>
													<td className="relative">
														...
														<DatePicker
															close={() => setCalendarOpen("")}
															open={calendarPopUp == "startDate" && !editStartDate}
